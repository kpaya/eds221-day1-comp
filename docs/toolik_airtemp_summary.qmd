...
```{r}
toolik_chlorophyll <- toolik_chlorophyll %>%
    clean_names()
```
...
ggplot(data = toolik_chlorophyll,
       aes(x=depth, y= active_chl_a_ug_l)) + geom_point(aes(col=active_chl_a_ug_l)) + theme_minimal() + labs(title="Toolik Chlorophyll Concentration", x="Lake Depth (m)", y=("Active Chlorophyll Concentration (mcg/L)"))
```
